...
#Load data with TMM values
setwd("")
data <- read.table("",sep = "\t",header = T, row.names=1)

#Prune insignificant data
#Remove rows if gene expression is less than .5 in 50% of samples, can be changed to be greater or lesser
...
#center columns so samples are centered on the same mean
scaled1 <- scale(pruned_data)


#transform data to be able to center on genes that have the same variance
t_scaled1 <- t(scaled1)
...
#pc1 and pc2 values are printed above grab the first two principle components values
data <- as.data.frame(datapca$x)
data <- data[,1:2]


#separate sample names into a table with each information in a new column
#add, remove, and change A,B,C to your own custom group sample names. 
...
#this section is used to subset sample groups for analysis (i.e A_1, A_2, B_1, B_2, C_1, C_2) 
#change 'seperator' to the value the seperates your group name from subgroup name
data_subset <- split(test1, test1$seperator, drop=FALSE)
